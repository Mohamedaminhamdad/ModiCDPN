    ...
    def forward(self, x): # x.shape [bs, 3, 256, 256]
        if self.freeze:
            with torch.no_grad():
                ...
                x = self.layer1(x_low_feature)  # x.shape [bs, 64, 64, 64]
                x = self.layer2(x)  # x.shape [bs, 128, 32, 32]
                x = self.layer3(x)  # x.shape [bs, 256, 16, 16]
                x_high_feature = self.layer4(x)  # x.shape [bs, 512, 8, 8]
